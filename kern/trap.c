	...
	while(~done){
		...
		if(normal){
			for(int i=curenv->page_last_WS_index;i<curenv->page_WS_max_size;i++){
				uint32 curr_ver = curenv->ptr_pageWorkingSet[i].virtual_address;
				uint32 *pageTable = NULL;
				struct Frame_Info *remVer_info = get_frame_info(curenv->env_page_directory, (void*)curr_ver, &pageTable);
				if((pageTable[PTX(curr_ver)] & PERM_USED)==0){
					if(pageTable[PTX(curr_ver)] & PERM_MODIFIED){
							pf_update_env_page(curenv, (void*)curr_ver, remVer_info);
					}
					unmap_frame(curenv->env_page_directory, (void*)curr_ver);
					curenv->ptr_pageWorkingSet[i].virtual_address = fault_va;
					done = 1 ;
					curenv->page_last_WS_index = (i+1)%curenv->page_WS_max_size;

					struct Frame_Info *newfinfo = get_frame_info(curenv->env_page_directory, (void*)fault_va, &pageTable);
					allocate_frame(&newfinfo);
					map_frame(curenv->env_page_directory, newfinfo, (void*)fault_va, PERM_USER|PERM_WRITEABLE);
					pf_read_env_page(curenv, (void*)fault_va);
					break;
				}
				else{
					pageTable[PTX(curr_ver)] &= (~PERM_USED);
				}
				curenv->page_last_WS_index = (i+1)%curenv->page_WS_max_size;
			}
		}
		// modified
			...
			for(int i=curenv->page_last_WS_index;i<curenv->page_WS_max_size;i++){
				...
				uint32 *pageTable = NULL;
				struct Frame_Info *remVer_info = get_frame_info(curenv->env_page_directory, (void*)curr_ver, &pageTable);
				if(((pageTable[PTX(curr_ver)] & PERM_USED)==0)&&((pageTable[PTX(curr_ver)] & PERM_MODIFIED)==0)){
					unmap_frame(curenv->env_page_directory, (void*)curr_ver);
					curenv->ptr_pageWorkingSet[i].virtual_address = fault_va;
					done = 1 ;
					curenv->page_last_WS_index = (i+1)%curenv->page_WS_max_size;
